...
int main(void)
{
  ...
  SystemClock_Config();
	...
	// select alternate functions for PC6, PC7
	// PC6 AF0 -> TIM3 CH1
	// PC7 AF0 -> TIM3 CH2
	GPIOC->AFR[0] &= ~(0xFF000000); // set both alternate functions to 0b0000 in the AFRL register
...
//	TIM2->CR1 |= (1 << 0); // set clock enable bit to start timer after setup
	...
	/*
	 * Set TIM3 frequency to 800Hz
	 *
	 * choose PSC+1 = 100, ARR = 100 -> 8000000/(100*100) = 8000000/10000
	 */
	 TIM3->ARR = 100; // 100
	 TIM3->PSC |= 0x63; // 99
	 TIM3->CCMR1 &= ~(1 << 0) & ~(1 << 1) & ~(1 << 8) & ~(1 << 9); // Set CCS1 and CCS2 to output
	 TIM3->CCMR1 |= (1 << 4) | (1 << 5) | (1 << 6); // set channel 1 to PWM mode 2
	 ...
	 TIM3->CCR1 = 20; // 20% of ARR for CH1 (red - 6)
	 TIM3->CCR2 = 20; // 20% of ARR for CH2 (blue - 7)
