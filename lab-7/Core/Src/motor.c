...
volatile int16_t prev_error_integral = 0;
...
void encoder_init(void) {
    ...
    // Select PSC and ARR values that give an appropriate interrupt rate - fTarget = fCLK/[ARR*(PSC+1)] = 22.22Hz -> every 0.045sec
    TIM6->PSC = 11;
    TIM6->ARR = 30000;
...
void PI_update(void) {
    ...
    /// TODO: calculate error signal and write to "error" variable
		// current TIM6 interrupt checks every 0.045sec
		// 64 counts * 50 GR = 3200 encoder counts per revolution
		// number of revolutions = encoder counts / (3200ct/rev)
		// rev/sec = rev/0.045
		// RPM = rev/sec * 60sec/min
		motor_speed_rpm = motor_speed / 2;
    ...
    /// TODO: Calculate integral portion of PI controller, write to "error_integral" variable
		...
		error_integral = prev_error_integral + (Ki * error);
